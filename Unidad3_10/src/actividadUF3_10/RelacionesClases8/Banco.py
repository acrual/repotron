...
class Banco(object):
    ...
    def traspasar(self, origen,destino,cantidad):
        posOrigen, enc = self.buscar(origen)
        posDestino, enc2 = self.buscar(destino)
        if enc and enc2:
            return self.cuentas[posOrigen].traspasar(self.cuentas[posDestino],cantidad)
        else:
            return False

    def liquidarIntereses(self):
        for i in range(self.cantidad):
            self.cuentas[i].pagarIntereses()
